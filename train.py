...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="image",
        help="The column of the dataset containing an good image.",
    )
    parser.add_argument(
        "--bad_image_column",
        type=str,
        default="image",
        help="The column of the dataset containing an bad image.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                bsz = good_noise.shape[0]

                timesteps = torch.randint(
                    0,
                    noise_scheduler.config.num_train_timesteps,
                    (bsz,),
                    device=good_model_input.device,
                )

                good_noisy_model_input = noise_scheduler.add_noise(
                    good_model_input, good_noise, timesteps
                )
                bad_noisy_model_input = noise_scheduler.add_noise(
                    bad_model_input, bad_noise, timesteps
                )
                ...
                bad_model_pred = unet(
                    bad_noisy_model_input,
                    timesteps,
                    prompt_embeds,
                    added_cond_kwargs=unet_added_conditions,
                ).sample
                good_model_ref = ref_unet(
                    good_noisy_model_input.half(),
                    timesteps,
                    prompt_embeds,
                    added_cond_kwargs=unet_added_conditions,
                ).sample
                bad_model_ref = ref_unet(
                    bad_noisy_model_input.half(),
                    timesteps,
                    prompt_embeds,
                    added_cond_kwargs=unet_added_conditions,
                ).sample
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    good_target = good_noise
                    bad_target = bad_noise
                elif noise_scheduler.config.prediction_type == "v_prediction":
                    good_target = noise_scheduler.get_velocity(
                        good_model_input, good_noise, timesteps
                    )
                    bad_target = noise_scheduler.get_velocity(
                        bad_model_input, bad_noise, timesteps
                    )
                elif noise_scheduler.config.prediction_type == "sample":
                    good_target = good_model_input
                    good_model_pred = good_model_pred - good_noise
                    bad_target = bad_model_input
                    bad_model_pred = bad_model_pred - bad_noise
                else:
                    raise ValueError(
